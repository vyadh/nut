...
export def tags []: string -> list<record<created: datetime, revision: string, tag: string>> {
    let path = $in
    cd $path

    let format = '{ created: "%(creatordate:iso8601-strict)" revision: "%(objectname)" tag : "%(refname:short)" }'
